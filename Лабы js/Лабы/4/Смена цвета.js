...
function initShaders() {
    ...
    if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) {
        alert("êŒêµ ëØê?ê°ê»êœëÆëæ ëØëÆë?ê°ê¡êœêßêﬁë?ëæ ë?êµê‹ê?êµë´ë?");
    }
...
function getShader(type, shader) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        alert("êÃë?êﬁê±êπê° êπêœê¿êªêﬁê»ë⁄ë†êﬁêﬁ ë?êµê‹ê?êµë´ê°: " + gl.getShaderInfoLog(shader));
...
var vertices = [
  ...
  var ëÆolors = [
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(ëÆolors), gl.STATIC_DRAW);
}
function draw() {    
...
window.onload=function(){
    ...
    try {
      ...
      if (!gl) {
        alert("ê’ê°ë? ê±ë´ê°ëØê?êµë´ ê¡êµ êªêœê?ê?êµë´ê¶êﬁêßê°êµë? WebGL");
      }
    if(gl){
